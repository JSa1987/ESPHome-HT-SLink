# ESPHome Home Theater Controller with Sony S-Link support

This is an ESPHome controller I made to fully control my home theater setup from Home Assistant. This controller includes the following input/outputs that suit my needs, but it can be easily modified for different setups.
- Sony S-Link Input/Output - Used to control a Sony CDP-CX225 CD Player
- IR Input - Used to detect ON/OFF and Volume commands from a Roku remote
- IR Output - Used to control a Yamaha RX-V2700 receiver and a daisy chained IR blaster
- ON/OFF Output (5v) - Used to control a power conditioner (completly turn off the power to the whole setup)
...
## Sony S-Link
An Arduino Pro Mini is used as interface for the Sony S-Link Bus. The Arduino is interfaced with the ESP8622 via a serial connection.
...
I'm successfully using this setup to control and read information from a Sony CDP-CX225 CD Player. The ESPHome yaml file with the configuration for this device is available in the `ESPHome` folder. 
...
The S-Link bus operates at a 5v logic level, while the ESP8266 operates at a 3.3v logic level. There are Arduino boards that operate either at 3.3v or at 5v, therefore the circuit will depend on the type of Arduino board used. The schematics above uses an the 3.3v version of the Arduino Pro Mini, therefore the Arduino can be connected directly to the ESP8266 but R2 and R3 are needed as voltage divider on the S-Link bus line. If an Arduino that operates at 5v logic level is used instead (e.g. Arduino Nano) then R2 and R3 won't be needed but a logic level shiftier will instead be needed between the Arduino and the ESP8266.
...
A 3.5 mm mono plug would be used also to connect to most IR blasters and IR input ports on receivers, the signal goes on the mono plug tip and the ground on the sleeve.
...
The ESPHome device also exposes the following services that can called from Home Assistant:
...
- cdp_bar => This will reflect the current position in the track been played as percentage of the track duration. 
```
template:
  - sensor:
      - name: "CD Album"
...
In addtion, the following Helpers need to be created in Home Assistant:
...
The following Scripts and Automatons are provided in the `Home Assistant` folder:
